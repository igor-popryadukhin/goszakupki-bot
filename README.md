...
3. При необходимости добавьте другие переменные из `src/config.py`.
   ...
   - `WEEKLY_REPORT_AT` — время отправки еженедельного отчёта (формат `HH:MM` в часовом поясе `TZ`). Если не задано, отчёт отправляется сразу после запуска и далее раз в 7 дней.
...
- Статистику за последние 7 дней можно запросить командой `/weekly_report`. Планировщик рассылает её автоматически раз в неделю всем авторизованным чатам.
